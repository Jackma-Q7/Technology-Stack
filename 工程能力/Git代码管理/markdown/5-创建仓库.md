...
&emsp;&emsp;在执行完成 **git init** 命令后，Git 仓库会生成一个 .git 目录，该目录包含了资源的所有元数据，其他的项目目录保持不变。
...
&emsp;&emsp;所以在 git bash 中 __**git commit -m '提交说明'**__ 这样是可以的，在 Windows 命令行中就要使用双引号 __**git commit -m "提交说明"**__。
...
&emsp;&emsp;执行该命令后，会在当前目录下创建一个名为grit的目录，其中包含一个 .git 的目录，用于保存下载下来的所有版本记录。

&emsp;&emsp;如果要自己定义要新建的项目目录名称，可以在上面的命令末尾指定新的名字：
