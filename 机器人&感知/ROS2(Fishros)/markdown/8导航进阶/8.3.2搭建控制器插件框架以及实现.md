...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2025/png/57223890/1765505068681-36a66826-e0e0-4ced-90ea-5ea7a8942f78.png)

**nav2_custom_controller / include / nav2_custom_controller / custom_controller.hpp**
...
getNearestTargetPose:

第一步就是根据当前位姿计算遍历路径，获取欧式距离最近的点的索引第二步则是擦除全局路径数组中从开始到最近点的数据，这样最近点就变成了索引为0的点第三步则根据剩余点的数据选取最近点作为目标点亦或是选取最近点的下一个点作为目标点

calculateAngleDifference:

计算目标点方向和当前机器人朝向之间的角度差
...
3.获取最近的目标点getNearestTargetPose 和计算角度差calculateAngleDifference4.根据角度差计算线速度和角速度->根据角度差计算速度，角度差大于 0.3 则原地旋转，否则直行
...
** nav2_custom_controller / CMakeLists.txt **
